...
## A sound player that plays each time a letter appears
@onready var voice_sfx_player: AudioStreamPlayer = $VoiceSfxPlayer

## Sounds that play that plays each time a letter appears
var voice_sfxs: Array[AudioStream] = []
...
func _on_dialogue_label_spoke(letter: String, letter_index: int, speed: float) -> void:
	# Skip playing sounds for empty spaces so it sounds natural
	if letter == " " or letter == "":
		return
		
	# If the sound is already playing from the previous fast letter, 
	# stop it instantly so it can restart cleanly
	if voice_sfx_player.playing:
		voice_sfx_player.stop()
	
	if not voice_sfxs.is_empty():
